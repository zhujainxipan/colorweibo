...
import com.ht.weibo.ui.widget.MyGridView;
import com.ht.weibo.util.FormateTimeUtil;
 ...
 *
 */
public class FavContentListAdapter extends BaseAdapter {
    ...
    public FavContentListAdapter(Context context, List<Favorite> favoriteList) {
    ...
    public int getItemViewType(int position) {
        //根据当前的数据，判断类型，并且返回
        Status status = favoriteList.get(position).status;
        int ret = 0;
        //下面的判断要和这里写的一致
                    ...
                    if (status != null) {
                        ...
                        if (retweetedStatus != null) {
                            ...
                            if (retweetedUser != null) {
                                zhuanFaName = retweetedUser.name;
                            }
                            String text = retweetedStatus.text;
                            if (text != null) {
                                //显示转发微博的内容
                                zhuanFaViewHolder.zhanFatxtContent.setText("@" + zhuanFaName + ":" + text);
                            }
